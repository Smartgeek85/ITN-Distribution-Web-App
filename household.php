...
<body class="bg-body-light dark:bg-dark-body">
    ...
    <div class="main-content group-data-[sidebar-size=lg]:xl:ml-[calc(theme('spacing.app-menu')_+_16px)] group-data-[sidebar-size=sm]:xl:ml-[calc(theme('spacing.app-menu-sm')_+_16px)] px-4 ac-transition">
        <div class="card">
            <h2 class="card-title">ITN distribution Form</h2>
        </div>
            ...
            <div class="col-span-full">
                <div class="card p-0">
                    <div class="flex-center-between p-6 pb-4 border-b border-gray-200 dark:border-dark-border">
                        <small class="text-lg card-title leading-none">Web form to collect ITN distribution data</small>
                       
                    </div>
    <form id="itnForm" class="p-6">
    <div class="flex lg:flex-row flex-col gap-x-4">
       
        <div class="w-full mb-4">
            <label for="household_name" class="form-label">Household Head Name</label>
            <input type="text" id="household_name" class="form-input" name="household_name" placeholder="Head Name" 
                autocomplete="off" required />
                  <input type="hidden" id="username" name="username" readonly value="<?php echo $username; ?>"
                autocomplete="off" required />
        </div>
    </div>
    <div class="flex lg:flex-row flex-col gap-x-4">
        <div class="w-full mb-4">
            ...
            <input type="number" id="family_members" class="form-input" name="family_members" autocomplete="off" required />
        </div>
        <div class="w-full mb-4">
            <label for="itns_distributed" class="form-label">ITNs Distributed</label>
            <input type="number" id="itns_distributed" class="form-input" readonly name="itns_distributed" />
        </div>
    </div>
...
<script>
// JavaScript for ITN Auto Calculation based on Family Members

document.getElementById('family_members').addEventListener('input', function() 
{
    const familyMembers = parseInt(this.value);
    const itnsDistributedInput = document.getElementById('itns_distributed');
    const errorMessage = document.getElementById('error-message');

    if (familyMembers <= 0 || isNaN(familyMembers)) {
        errorMessage.textContent = "Household not eligible for ITNs.";
        itnsDistributedInput.value = '';
        return;
    }

    errorMessage.textContent = ''; // Clear error

    // Calculate ITNs based on family members
    if (familyMembers >= 1 && familyMembers <= 2) {
        itnsDistributedInput.value = 1;
    } else if (familyMembers >= 3 && familyMembers <= 4) {
        itnsDistributedInput.value = 2;
    } else if (familyMembers >= 5) {
        itnsDistributedInput.value = 3;
    }
});

// JavaScript to handle form submission with the Fetch API
document.getElementById('itnForm').addEventListener('submit', function(event)
{
    event.preventDefault();

  const username = document.getElementById('username').value;
    const household_name = document.getElementById('household_name').value;
    const family_members = document.getElementById('family_members').value;
    const itns_distributed = document.getElementById('itns_distributed').value;

    // Validation
    if (!household_name || !family_members || !itns_distributed) 
    {
        document.getElementById('error-message').textContent = "Please fill all fields correctly.";
        return;
    }
    ...
    const data = 
    {
        ...
        family_members: parseInt(family_members),
        itns_distributed: parseInt(itns_distributed)
    };
    ...
    .then(result => {
        if (result.error) {
            ...
            document.getElementById('success-message').textContent = "Data successfully submitted.";
        }
    })
    .catch(error => {
        document.getElementById('error-message').textContent = "Error submitting data.";
